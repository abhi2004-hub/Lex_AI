...
function setupFileUpload() {
    ...
    const browseButton = uploadZone.querySelector('.btn.btn-primary');
...
function showDetailedClauseAnalysis(clause) {
    ...
    if (explanationContent) {
        explanationContent.innerHTML = `
            ...
            <strong>Analysis:</strong> ${clause.risk_assessment?.recommendations || 'Standard clause with no significant risks identified.'}<br><br>
    ...
    try {
        ...
        shapContainer.innerHTML = `
            ...
            <div class="shap-chart">
                ${shapData.words.map((word, index) => `
                    <div class="shap-item">
                        <span class="shap-word">${word}</span>
                        <div class="shap-bar-container">
                            <div class="shap-bar ${shapData.values[index] >= 0 ? 'positive' : 'negative'}" 
                                 style="width: ${Math.abs(shapData.values[index]) * 100}%"></div>
                        </div>
                        <span class="shap-value">${shapData.values[index].toFixed(3)}</span>
                    </div>
                `).join('')}
            });
        }, 100);
...
function updateRiskChart(analysisData) {
    ...
    // Get risk distribution from backend analysis
    const riskDistribution = analysisData.analysis ? 
        (analysisData.analysis.risk_distribution || { low: 65, medium: 25, high: 10, critical: 0 }) : 
        { low: 65, medium: 25, high: 10, critical: 0 };
